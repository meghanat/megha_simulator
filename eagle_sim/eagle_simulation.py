...
class Job(object):
    ...
    def __init__(
        ...
        # dephase the incoming job in case it has the exact submission time as
        # another already submitted job
        ...
        if estimate_distribution == EstimationErrorDistribution.MEAN:
            ...
            self.estimated_task_duration = int(
                mean_task_duration + off_mean_top * mean_task_duration)
        ...
        self.remaining_exec_time = self.estimated_task_duration * \
            len(self.unscheduled_tasks)

    # Job class    """ Returns true if the job has completed, and false
    # otherwise. """
...
class JobArrival(Event, file):
    ...
    def run(self, current_time):
        ...
        if not long_job:
            ...
            rnd_worker_in_big_partition = random.randint(
                self.simulation.index_first_worker_of_big_partition, len(
                    self.simulation.workers) - 1)
            ...
            if SYSTEM_SIMULATED == "IdealEagle":
                possible_worker_indices = self.simulation.get_list_non_long_job_workers_from_btmap(
                    self.simulation.cluster_status_keeper.get_btmap())

            worker_indices = self.simulation.find_workers_random(
                PROBE_RATIO, self.job.num_tasks, possible_worker_indices, MIN_NR_PROBES)
            ...
            if (SYSTEM_SIMULATED == "DLWL"):
                ...
                estimated_task_durations = [self.job.estimated_task_duration for i in range(
                    len(self.job.unscheduled_tasks))]
                worker_indices = self.simulation.find_workers_dlwl(
                    estimated_task_durations,
                    self.simulation.shared_cluster_status,
                    current_time,
                    self.simulation,
                    possible_workers)
                self.simulation.cluster_status_keeper.update_workers_queue(
                    worker_indices, True, self.job.estimated_task_duration)
                ...
                worker_indices = self.simulation.find_workers_long_job_prio(
                    self.job.num_tasks,
                    self.job.estimated_task_duration,
                    workers_queue_status,
                    current_time,
                    self.simulation,
                    possible_workers)
                self.simulation.cluster_status_keeper.update_workers_queue(
                ...
                worker_indices = self.simulation.find_workers_random(
                    PROBE_RATIO,
                    self.job.num_tasks,
                    self.simulation.big_partition_workers,
                    MIN_NR_PROBES)
        ...
        new_events = self.simulation.send_probes(
            self.job, current_time, worker_indices, btmap)
        ...
        self.job = Job(
            ...
            line,
            self.job.estimate_distribution,
            self.job.off_mean_bottom,
            self.job.off_mean_top)
...
class PeriodicTimerEvent(Event):
    ...
    def run(self, current_time):
        ...
        total_load = str(int(10000 *
                             (1 -
                              self.simulation.total_free_slots *
                              1.0 /
                              (TOTAL_WORKERS *
                               SLOTS_PER_WORKER))) /
                         100.0)
        small_load = str(int(10000 *
                             (1 -
                              self.simulation.free_slots_small_partition *
                              1.0 /
                              len(self.simulation.small_partition_workers))) /
                         100.0)
        big_load = str(int(10000 *
                           (1 -
                            self.simulation.free_slots_big_partition *
                            1.0 /
                            len(self.simulation.big_partition_workers))) /
                       100.0)
        small_not_big_load = "N/A"
        if(len(self.simulation.small_not_big_partition_workers) != 0):
            small_not_big_load = str(int(10000 *
                                         (1 -
                                          self.simulation.free_slots_small_not_big_partition *
                                          1.0 /
                                          len(self.simulation.small_not_big_partition_workers))) /
                                     100.0)

        print >> load_file, "total_load: " + total_load + " small_load: " + small_load + " big_load: " + \
            big_load + " small_not_big_load: " + small_not_big_load + " current_time: " + str(current_time)
...
class WorkerHeartbeatEvent(Event):
    ...
    def run(self, current_time):
        ...
        if(HEARTBEAT_DELAY == 0):
            ...
            self.simulation.shared_cluster_status = copy.deepcopy(
                self.simulation.cluster_status_keeper.get_queue_status())
...
class ProbeEvent(Event):
    ...
    def run(self, current_time):
        return self.worker.add_probe(
            self.job_id,
            self.task_length,
            self.job_type_for_scheduling,
            current_time,
            self.btmap,
            False)
...
class ClusterStatusKeeper():
    ...
    def update_workers_queue(self, worker_indices, increase, duration):
        for worker in worker_indices:
            ...
            assert queue >= 0, (" offending value for queue: %r %i " % (
                queue, worker))
...
class TaskEndEvent():
    ...
    def run(self, current_time):
        ...
        if(self.job_type_for_scheduling != BIG):
            ...
            self.status_keeper.update_workers_queue(
                [self.worker.id], False, self.estimated_task_duration)
...
class Worker(object):
    def __init__(
        ...
        # List of times when slots were freed, for each free slot (used to
        # track the time the worker spends idle).
        self.free_slots = []
        ...
        if (id <= index_last_small):
            self.in_small = True
        if (id >= index_first_big):
            self.in_big = True
        if (id < index_first_big):
            self.in_small_not_big = True
    ...
    def add_probe(
        ...
        if (not long_job and handle_stealing == False and (
                self.executing_big == True or self.queued_big > 0)):
        ...
        self.queued_probes.append(
            [job_id, task_length, (self.executing_big or self.queued_big > 0), 0, False, -1])
    ...
    def free_slot(self, current_time):
        ...
        if len(self.queued_probes) == 0 and self.simulation.stealing_allowed:
    ...
    def ask_probes(self, current_time):
        ...
        while(ctr_it < STEALING_ATTEMPTS and len(new_probes) == 0):
            from_worker, new_probes = self.simulation.get_probes(
        ...
        for job_id, task_length, behind_big, cum, sticky, handle_steal in new_probes:
            assert (
                self.simulation.jobs[job_id].job_type_for_comparison != BIG)
            new_events.extend(
                self.add_probe(
                    job_id,
                    task_length,
    ...
    def get_probes_atc(self, current_time, free_worker_id):
        ...
        if not self.executing_big:
            # self.queued_probes[i][1] <= CUTOFF_THIS_EXP):
            while (i < len(self.queued_probes)
                   and self.simulation.jobs[self.queued_probes[i][0]].job_type_for_comparison != BIG):
                i += 1
        ...
        if (i < len(self.queued_probes)):
            while (len(self.queued_probes) > i and self.simulation.jobs[self.queued_probes[i]
                                                                        [0]].job_type_for_scheduling != BIG and nr_short_chosen < STEALING_LIMIT):
                nr_short_chosen += 1
    ...
    def get_probes_random(self, current_time, free_worker_id):
        ...
        while (i < len(self.queued_probes)):
            big_job = self.simulation.jobs[self.queued_probes[i]
                                           [0]].job_type_for_scheduling == BIG
        ...
        i = 0
        while (len(all_positions_of_short_tasks) > 0 and len(
                randomly_chosen_short_task_positions) < STEALING_LIMIT):
            rnd_index = random.randint(
                0, len(all_positions_of_short_tasks) - 1)
            randomly_chosen_short_task_positions.append(
                all_positions_of_short_tasks.pop(rnd_index))
        randomly_chosen_short_task_positions.sort()

        # remove the selected probes from the worker queue in decreasing order
        # of IDs
        decreasing_ids = len(randomly_chosen_short_task_positions) - 1
        total_time_probes = 0
        while (decreasing_ids >= 0):
            probes_to_give.append(
                self.queued_probes.pop(
                    randomly_chosen_short_task_positions[decreasing_ids]))
            total_time_probes += probes_to_give[-1][1]
            decreasing_ids -= 1
    ...
    def process_next_probe_in_the_queue(self, current_time):
        ...
        if (SRPT_ENABLED):
            if (SYSTEM_SIMULATED == "Eagle" or SYSTEM_SIMULATED == "Hawk"):
                ...
                if (pos == -1):
                    ...
                    # Cannot just return [], free_slot would not be called
                    # again
        ...
        was_successful, events = self.simulation.get_task(
            job_id, self, current_time)
        job_bydef_big = (
            self.simulation.jobs[job_id].job_type_for_comparison == BIG)
        ...
        if(not job_bydef_big and self.queued_probes[0][2] and self.in_big and was_successful):
        ...
        if(SBP_ENABLED and was_successful and not job_bydef_big):
        ...
        if(SRPT_ENABLED and (SYSTEM_SIMULATED == "Eagle" or SYSTEM_SIMULATED == "Hawk")):
            ...
            if(was_successful and not job_bydef_big):
                ...
                for delay_it in range(0, pos):
                    ...
                    new_acc_delay = self.queued_probes[delay_it][3] + \
                        estimated_task_duration
                    ...
                    if(SYSTEM_SIMULATED == "Eagle"):
                        ...
                        if (CAP_SRPT_SBP != float('inf')):
                            assert(new_acc_delay <= CAP_SRPT_SBP * task_duration), " Chosen position: %r, length %r , offending  %r" % (
                                pos, len(self.queued_probes), new_acc_delay)
                        else:
                            assert(new_acc_delay <= CAP_SRPT_SBP and new_acc_delay >= 0), " Chosen position: %r, length %r , offending  %r" % (
                                pos, len(self.queued_probes), new_acc_delay)
    ...
    def get_remaining_exec_time_for_job(self, job_id, current_time):
        ...
        if (len(job.unscheduled_tasks) == 0):  # Improvement
            ...
            remaining_exec_time = len(
                job.unscheduled_tasks) * job.estimated_task_duration
        return remaining_exec_time
    ...
    def get_next_probe_acc_to_sbp_srpt(self, current_time):
        ...
        while position_it < len_queue:
            ...
            remaining = self.get_remaining_exec_time_for_job_dist(
                job_id, current_time)
            ...
            if (remaining == -1):
                ...
                if (CAP_SRPT_SBP != float('inf')):
                    slack = CAP_SRPT_SBP * estimated_task_duration - \
                        self.queued_probes[position_it][3]
                    if(CAP_SRPT_SBP == 0):
                        assert(self.queued_probes[position_it][3] == 0)
                    assert (slack >= 0), " offending value slack: %r CAP %r" % (
                        slack, CAP_SRPT_SBP)
    ...
    def get_next_probe_acc_to_srpt(self, current_time):
        ...
        for position_it in range(0, len(self.queued_probes)):
            job_id = self.queued_probes[position_it][0]
            remaining = self.get_remaining_exec_time_for_job(
                job_id, current_time)
            assert(remaining >= 0)
            ...
            # calculate shortest slack for next probes
            if (CAP_SRPT_SBP != float('inf')):
                # will be negative is estimated_task_duration = 0
                slack = CAP_SRPT_SBP * estimated_task_duration - \
                    self.queued_probes[position_it][3]
                # improvement: if its 0, no task will be allowed to bypass it
                if(slack < shortest_slack_in_front):
...
class Simulation(object):
    def __init__(
        ...
        self.index_last_worker_of_small_partition = int(
            SMALL_PARTITION * TOTAL_WORKERS * SLOTS_PER_WORKER / 100) - 1
        self.index_first_worker_of_big_partition = int(
            (100 - BIG_PARTITION) * TOTAL_WORKERS * SLOTS_PER_WORKER / 100)

        while len(self.workers) < TOTAL_WORKERS:
            self.workers.append(Worker(self,
                                       SLOTS_PER_WORKER,
                                       len(self.workers),
                                       self.index_last_worker_of_small_partition,
                                       self.index_first_worker_of_big_partition))
        ...
        # so not including the worker after :
        self.small_partition_workers = self.worker_indices[:
        ...
        # so including the worker before :
        self.big_partition_workers = self.worker_indices[self.index_first_worker_of_big_partition:]
        for node in self.big_partition_workers:
            self.big_partition_workers_hash[node] = 1

        # so not including the worker after:
        self.small_not_big_partition_workers = self.worker_indices[
            :self.index_first_worker_of_big_partition]
        ...
        self.free_slots_small_not_big_partition = len(
            self.small_not_big_partition_workers)
    ...
    def find_workers_random(
            self,
            probe_ratio,
            nr_tasks,
            possible_worker_indices,
            min_probes):
    ...
    def find_workers_long_job_prio(
            self,
            num_tasks,
            estimated_task_duration,
            workers_queue_status,
        ...
        for index in hash_workers_considered:
            ...
            if qlen == 0:
                ...
                if(start_of_crt_big_task != -1):
                    ...
                    adjusted_waiting_time = 2 * NETWORK_DELAY + \
                        qlen - min(executed_so_far, estimated_crt_task)
    ...
    def find_workers_dlwl(
        ...
        for index in hash_workers_considered:
            ...
            adjusted_waiting_time = qlen
            # PAMELA: it will always be considered big for DLWL
            start_of_crt_big_task = worker_obj.tstamp_start_crt_big_task
            assert(current_time >= start_of_crt_big_task)
            ...
            if(start_of_crt_big_task != -1):
                executed_so_far = current_time - start_of_crt_big_task
                estimated_crt_task = worker_obj.estruntime_crt_task
                adjusted_waiting_time = max(
                    2 * NETWORK_DELAY + qlen - min(executed_so_far, estimated_crt_task), 0)
        ...
        while (workers_needed > len(chosen_worker_indices)):
            ...
            while(queue_length <= next_queue_length and len(chosen_worker_indices) < workers_needed):
                chosen_worker_indices.append(worker)
                # copy_estimated_task_durations.pop(0)
                queue_length += estimated_task_durations[0]
    ...
    def send_probes(self, job, current_time, worker_indices, btmap):
        if SYSTEM_SIMULATED == "Hawk" or SYSTEM_SIMULATED == "IdealEagle":
            return self.send_probes_hawk(
                job, current_time, worker_indices, btmap)
        if SYSTEM_SIMULATED == "Eagle":
            return self.send_probes_eagle(
                job, current_time, worker_indices, btmap)
            ...
            probe_events.append(
                (current_time + NETWORK_DELAY,
                 ProbeEvent(
                     ...
                     job.id,
                     job.estimated_task_duration,
                     job.job_type_for_scheduling,
                     btmap)))
    ...
    def try_round_of_probing(
        ...
        for worker_index in worker_list:
            ...
            if (self.workers[worker_index].btmap_tstamp >
                    freshest_btmap_tstamp):
        ...
        if (id_worker_with_newest_btmap != -1):
            self.btmap = copy.deepcopy(
                self.workers[id_worker_with_newest_btmap].btmap)
    ...
    def send_probes_eagle(self, job, current_time, worker_indices, btmap):
        ...
        if (job.job_type_for_scheduling == BIG):
            ...
            for i in range(0, ROUNDS_SCHEDULING):
                ok_nr, ok_nodes = self.try_round_of_probing(
                    current_time, job, worker_indices, probe_events, i + 1)
                if(i == 0):
                ...
                list_non_long_job_workers = self.get_list_non_long_job_workers_from_btmap(
                    self.btmap)
                # small
                ...
                #   break
                worker_indices = self.find_workers_random(
                    1, missing_probes, list_non_long_job_workers, 0)
                stats.STATS_REASSIGNED_PROBES += missing_probes

            if(missing_probes > 0):
                worker_indices = self.find_workers_random(
                    1, missing_probes, self.small_not_big_partition_workers, 0)
                self.try_round_of_probing(
                    current_time,
                    job,
                    worker_indices,
                    probe_events,
    ...
    def increase_free_slots_for_load_tracking(self, worker):
        ...
        if(worker.in_small):
            self.free_slots_small_partition += 1
        if(worker.in_big):
            self.free_slots_big_partition += 1
        if(worker.in_small_not_big):
            self.free_slots_small_not_big_partition += 1
    ...
    def get_task(self, job_id, worker, current_time):
        ...
        # account for the fact that this is called when the probe is launched
        # but it needs an RTT to talk to the scheduler
        get_task_response_time = current_time + 2 * NETWORK_DELAY

        if len(job.unscheduled_tasks) == 0:
            return False, [
                (get_task_response_time, NoopGetTaskResponseEvent(worker))]
        ...
        is_job_complete = job.update_task_completion_details(
            task_completion_time)

        if is_job_complete:
            self.jobs_completed += 1
            print >> finished_file, task_completion_time, " estimated_task_duration: ", job.estimated_task_duration, " by_def: ", job.job_type_for_comparison, " total_job_running_time: ", (
        ...
        events.append(
            (task_completion_time,
             TaskEndEvent(
                 worker,
                 self.SCHEDULE_BIG_CENTRALIZED,
                 self.cluster_status_keeper,
                 job.id,
                 job.job_type_for_scheduling,
                 job.estimated_task_duration,
                 this_task_id)))
    ...
    def get_probes(self, current_time, free_worker_id):
        worker_index = random.randint(
            self.index_first_worker_of_big_partition, len(
                self.workers) - 1)
        if (STEALING_STRATEGY == "ATC"):
            probes = self.workers[worker_index].get_probes_atc(
                current_time, free_worker_id)
        elif (STEALING_STRATEGY == "RANDOM"):
            probes = self.workers[worker_index].get_probes_random(
                current_time, free_worker_id)
    ...
    def run(self):
        ...
        self.event_queue.put(
            (float(line.split()[0]), PeriodicTimerEvent(self)))
...
# cap on the nr of tasks to steal from one node
STEALING_LIMIT = int(sys.argv[15])
# cap on the nr of nodes to contact for stealing
STEALING_ATTEMPTS = int(sys.argv[16])
...
print >> stats_file, "STATS_SHORT_JOBS_HAVING_TASKS_THAT_WAITED_FOR_BIG:         ", len(
    s.jobs_affected_by_holb)
print >> stats_file, "                                 "
...
if(stats.STATS_ROUNDS_ATTEMPTS != 0):
    print >> stats_file, "STATS_ROUNDS/STATS_ROUNDS_ATTEMPTS:             ", stats.STATS_ROUNDS * \
        1.0 / stats.STATS_ROUNDS_ATTEMPTS
    print >> stats_file, "STATS_PERC_1ST_ROUNDS/STATS_ROUNDS_ATTEMPTS:    ", stats.STATS_PERC_1ST_ROUNDS * \
        1.0 / stats.STATS_ROUNDS_ATTEMPTS
print >> stats_file, "                                 "
...
if(SYSTEM_SIMULATED == "Eagle"):
    print >> stats_file, "%TASKS AFFECTED BY HOLB ", stats.STATS_SHORT_TASKS_WAITED_FOR_BIG * 1.0 / \
        stats.STATS_TASKS_SHORT_FINISHED, " %JOBS ", len(s.jobs_affected_by_holb) * 1.0 / s.jobs_scheduled
