    ...
    def run(self, current_time):
        ...
        self.simulation.event_queue.put(
            (current_time +
             ...
             NETWORK_DELAY,
             TaskEndEvent(
                 self.task)))  # launching requires network transfer
...
class MatchFoundEvent(Event):
    ...
    def run(self, current_time):
        ...
        print(current_time, ",", "MatchFoundEvent", ",", self.task.job.job_id +
              "_" + self.task.task_id, ",", self.gm.GM_id + "_" + str(self.node_id))
...
class Job(object):
    ...
    def __init__(self, task_distribution, line, simulation):
        ...
        if self.start_time not in job_start_tstamps:  # IF the job's start_time has never been seen before
            ...
            # Shift the start time of the jobs with this duplicate start time
            # by 0.01s forward to prevent a clash
    ...
    def file_task_execution_time(self, job_args):
        for task_duration in (
                job_args[3:]):  # Adding each of the tasks to the dict
...
class LM(object):
    ...
    def get_status(self, gm):
        ...
        response = [json.dumps(self.LM_config),
                    json.dumps(self.tasks_completed[gm.GM_id])]
    ...
    # tasks completed for the corresponding GM that sent it
        ...
        # update from node to LM - 1 NETWORK_DELAY + update from LM to GM - 1
        # NETWORK DELAY = 2 NETWORK_DELAYS
        self.simulation.event_queue.put(
            (task.end_time + (2 * NETWORK_DELAY), LMUpdateEvent(self.simulation, periodic=False)))
                                ...
                                self.simulation.event_queue.put(
                                    (current_time,
                                     MatchFoundEvent(
                                         ...
                                         node_id,
                                         current_time,
                                         external_partition=GM_id)))  # may need to add processing overhead here if required
                                matchfound = True
                                break
                        ...
                        self.simulation.event_queue.put(
                            (current_time,
                             MatchFoundEvent(
                                 ...
                                 self.simulation.lms[LM_id],
                                 node_id,
                                 current_time)))  # may need to add processing overhead here if required
                        matchfound = True
                        break
                if(matchfound):  # If this task was successfully placed then, move on to the next task
                    continue
...
class Simulation(object):
    def __init__(
        ...
        while len(self.gms) < self.NUM_GMS:
            self.gms[str(counter)] = GM(self, str(counter),
                                        pickle.loads(pickle.dumps(self.config)))
            counter += 1
        ...
        while len(self.lms) < self.NUM_LMS:
            self.lms[str(counter)] = LM(self, str(counter), PARTITION_SIZE, pickle.loads(
                pickle.dumps(self.config["LMs"][str(counter)])))
            counter += 1
...
# not simulation's virtual time. This is just to understand how long the
# program takes
t1 = time.time()
s = Simulation(
    WORKLOAD_FILE,
